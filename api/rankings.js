...
export default async function handler(req, res) {
  ...
  try {
    ...
    const rankings = [...byUser.values()]
      ...
      .map((row,index) => ({ rank:index+1, ...row, totalTime:formatSeconds(row.totalTimeSeconds) }));
    return res.status(200).json({ category, rankings });
  } catch (error) { console.error(error); return res.status(500).json({ error: 'Could not load rankings.' }); }
}
