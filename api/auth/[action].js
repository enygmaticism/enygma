import { readJsonFile, writeJsonFile } from '../../lib/github.js';
import { createPasswordRecord, encryptUsers, normalizeUsername, validUsername, readUsers, verifyPassword, setSession, clearSession, getSessionUser } from '../../lib/auth.js';
...
export default async function handler(req, res) {
  ...
  if (action === 'logout') { if(req.method!=='POST'){res.setHeader('Allow','POST');return res.status(405).json({error:'Method not allowed'});} clearSession(res); return res.status(200).json({ok:true}); }
  if (action === 'me') { if(req.method!=='GET'){res.setHeader('Allow','GET');return res.status(405).json({error:'Method not allowed'});} const session=getSessionUser(req); if(!session)return res.status(401).json({authenticated:false}); return res.status(200).json({authenticated:true,username:session.username}); }
