...
function secretKey(){const secret=process.env.ADMIN_PASSWORD||'';if(!secret)throw new Error('ADMIN_PASSWORD is not configured.');return crypto.createHash('sha256').update(`enygma-user-data:${secret}`).digest();}
...
function sign(payload){const encoded=Buffer.from(JSON.stringify(payload)).toString('base64url'),signature=crypto.createHmac('sha256',secretKey()).update(encoded).digest('base64url');return`${encoded}.${signature}`;}
function verify(token){try{const[encoded,signature]=String(token||'').split('.');if(!encoded||!signature)return null;const expected=crypto.createHmac('sha256',secretKey()).update(encoded).digest('base64url'),a=Buffer.from(signature),b=Buffer.from(expected);if(a.length!==b.length||!crypto.timingSafeEqual(a,b))return null;const payload=JSON.parse(Buffer.from(encoded,'base64url').toString('utf8'));if(!payload.exp||payload.exp<Math.floor(Date.now()/1000))return null;return payload;}catch{return null;}}
...
export async function readUsers(readJsonFile){const{data}=await readJsonFile('data/users.secure.json',{version:1,ciphertext:''});if(!data.ciphertext)return{users:{},sha:null};const payload=Buffer.from(data.ciphertext,'base64'),iv=payload.subarray(0,12),tag=payload.subarray(payload.length-16),ciphertext=payload.subarray(12,payload.length-16),decipher=crypto.createDecipheriv('aes-256-gcm',secretKey(),iv);decipher.setAuthTag(tag);const plaintext=Buffer.concat([decipher.update(ciphertext),decipher.final()]).toString('utf8');return{users:JSON.parse(plaintext),sha:null};}
export function encryptUsers(users){const iv=crypto.randomBytes(12),cipher=crypto.createCipheriv('aes-256-gcm',secretKey(),iv),ciphertext=Buffer.concat([cipher.update(JSON.stringify(users),'utf8'),cipher.final()]),tag=cipher.getAuthTag();return Buffer.concat([iv,ciphertext,tag]).toString('base64');}
