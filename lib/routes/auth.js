...
async function loadUsers() {
  return readUsers(readJsonFile);
}

async function migrateUsers(store) {
  if (store.keyType !== 'legacy' || !store.sha) return;
  await writeJsonFile('data/users.secure.json',{version:1,ciphertext:encryptUsers(store.users)},store.sha,'Migrate player account encryption');
}
...
export default async function handler(req, res) {
  ...
  if (action === 'register') {
    ...
    try {
      ...
      const store=await loadUsers(), users=store.users;
      ...
      await writeJsonFile('data/users.secure.json',{version:1,ciphertext:encryptUsers(users)},store.sha,`Create player account: ${username}`);
      setSession(res,username); return res.status(201).json({ok:true,username});
    } catch(error){ console.error(error); return res.status(500).json({error:'Could not create account.'}); }
  }
