...
async function load(category) {
  ...
  try {
    ...
    list.innerHTML = result.rankings.length ? result.rankings.map(row => `<div class="ranking-row"><span class="rank-number">${row.rank}</span><span class="rank-user">${row.username}</span><span>${row.solved}</span><strong>${Number(row.points).toLocaleString()}</strong></div>`).join('') : '<div class="empty">No results yet.</div>';
  } catch (error) {
