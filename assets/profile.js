...
async function load() {
  try {
    const response = await fetch('api/results', { credentials: 'same-origin', cache: 'no-store' });
    ...
    document.getElementById('record-grid').innerHTML = Object.entries(LABELS).map(([key, label]) => {
      ...
      return `<article class="record-card">
        ...
        <div class="record-main"><strong>${stat.solved}</strong><span>solved</span></div>
        <dl><div><dt>Average time</dt><dd>${formatTime(stat.averageTimeSeconds)}</dd></div><div><dt>Points</dt><dd>${Number(stat.points).toLocaleString()}</dd></div>${key === 'connections' ? `<div><dt>Categories solved</dt><dd>${stat.categoriesSolved}</dd></div>` : ''}</dl>
      </article>`;
    }).join('');
